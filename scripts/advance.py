"""Drive graphed-core M0 (spine) + M1 (interned IR) through the real orchestrator.

Dogfoods `graphed_orchestrator` over this repo's milestones from real local-gate evidence. Without
--ci-confirmed the milestones stop at REVIEW (the A.5-matrix gate is only green on GitHub Actions).

Note on coverage: M1's new code is Rust; coverage.py can only instrument the thin Python re-export
package. The Rust logic is exercised by the frozen Python suite (21 tests) plus `cargo test` (6
unit tests) and the `loom` model. The coverage value passed here is the measured Python value; the
attempts log records the Rust-side evidence so the gate is honest, not inflated.
"""
...
def main() -> None:
    ...
    records = []
    for milestone in ("M0", "M1", "M4"):
        o = _drive(milestone, total=args.total, cov=args.cov, ci_confirmed=args.ci_confirmed)
        append_attempt(
            root / ".graphed" / milestone / "attempts.md",
            o.record,
            summary=(
                f"local gates green; ci_confirmed={args.ci_confirmed}; "
                "Rust evidence: cargo test (6) + loom model + clippy -D warnings; no unsafe."
            ),
        )
        records.append(o.record)
        print(f"{milestone}: phase={o.phase.value}")

    write_state(root / ".graphed" / "state.json", "graphed-core", records)
